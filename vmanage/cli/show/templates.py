...
@click.option('--type',
              ...
              default=None)
@click.option('--diff',
              help="Diff with template of specified name",
              default=None)
@click.option('--default/--no-default',
              help="Print system default templates",
              default=False)
@click.option('--name', '-n')
...
def templates(ctx, type, name, diff, default, json):
    ...
    if name:
        if type == 'device':
            template_list = device_templates.get_device_template_list(
                name_list=[name])
        elif type == 'feature':
            template_list = feature_templates.get_feature_template_list(
                name_list=[name])
        else:
        ...
        if template:
            if diff:
                diff_template = {}
                if type == 'device':
                    diff_template_list = device_templates.get_device_template_list(
                        name_list=[diff])
                    if diff_template_list:
                        diff_template = diff_template_list[0]
                    else:
                        click.secho(f"Cannot find device template {diff}",
                                    fg="red")
                elif type == 'feature':
                    diff_template_list = feature_templates.get_feature_template_list(
                        name_list=[diff])
                    if diff_template_list:
                        diff_template = diff_template_list[0]
                    else:
                        click.secho(f"Cannot find feature template {diff}",
                                    fg="red")
                else:
                    # Should not get here
