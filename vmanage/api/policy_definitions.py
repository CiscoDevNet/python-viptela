...
class PolicyDefinitions(object):
        ...
        if isinstance(input, dict):
            for key, value in list(input.items()):
                if key.endswith('List'):
                    ...
                    policy_list_dict = self.policy_lists.get_policy_list_dict(
                        type, key_name='listId')
                    ...
                    if input['ref'] in policy_list_dict:
                        input['listName'] = policy_list_dict[
                            input['ref']]['name']
                        input['listType'] = policy_list_dict[
                            input['ref']]['type']
                        input.pop('ref')
                    else:
                        raise Exception("Could not find list {0}".format(
                            input['ref']))
                elif key == 'class':
                    policy_list_dict = self.policy_lists.get_policy_list_dict(
                        'all', key_name='listId')
                    if input['class'] in policy_list_dict:
                        input['className'] = policy_list_dict[
                            input['class']]['name']
                        input['classType'] = policy_list_dict[
                            input['class']]['type']
                        input.pop('class')
                    else:
                        raise Exception("Could not find list {0}".format(
            ...
            for key, value in list(input.items()):
                if key.endswith('List'):
                    ...
                    if 'listType' in input:
                        policy_list_dict = self.policy_lists.get_policy_list_dict(
                            input['listType'], key_name='name')
                    else:
                        raise Exception(
                            "Could not find type for list {0}".format(
                                input['listName']))
                    if input[
                            'listName'] in policy_list_dict and 'listId' in policy_list_dict[
                                input['listName']]:
                        input['ref'] = policy_list_dict[
                            input['listName']]['listId']
                        input.pop('listName')
                        input.pop('listType')
                    else:
                        raise Exception(
                            "Could not find id for list {0}, type {1}".format(
                                input['listName'], input['listType']))
                elif key == 'className':
                    if 'classType' in input:
                        policy_list_dict = self.policy_lists.get_policy_list_dict(
                            input['classType'], key_name='name')
                    else:
                        raise Exception(
                            "Could not find type for list {0}".format(
                                input['className']))
                    if input[
                            'className'] in policy_list_dict and 'listId' in policy_list_dict[
                                input['className']]:
                        input['class'] = policy_list_dict[
                            input['className']]['listId']
                        input.pop('className')
                        input.pop('classType')
                    else:
                        raise Exception(
    ...
    def convert_definition_id_to_name(self, policy_definition):
        if 'assembly' in policy_definition:
            for assembly_item in policy_definition['assembly']:
                policy_definition_detail = self.get_policy_definition(
                    assembly_item['type'].lower(),
                    assembly_item['definitionId'])
                definition_id = assembly_item.pop('definitionId')
                if policy_definition_detail:
                    assembly_item['definitionName'] = policy_definition_detail[
                        'name']
                else:
                    raise Exception(
                        "Cannot find policy definition for {0}".format(
                            definition_id))
    ...
    def convert_definition_name_to_id(self, policy_definition):
        if 'assembly' in policy_definition:
            for assembly_item in policy_definition['assembly']:
                ...
                policy_definition_dict = self.get_policy_definition_dict(
                    assembly_item['type'])
                if definition_name in policy_definition_dict:
                    assembly_item['definitionId'] = policy_definition_dict[
                        definition_name]['definitionId']
                else:
                    raise Exception("Cannot find policy definition {0}".format(
                        definition_name))
                if 'entries' in assembly_item:
    ...
    def add_policy_definition(self, policy_definition):
        ...
        response = HttpMethods(self.session, url).request(
            'POST', payload=json.dumps(policy_definition))

    def update_policy_definition(self, policy_definition,
        ...
        response = HttpMethods(self.session, url).request(
            'PUT', payload=json.dumps(policy_definition))
    ...
    def get_policy_definition_list(self, definition_type='all'):
        ...
        if definition_type == 'all':
            ...
            for definition_type in definition_list_types:
                definition_list = self.get_policy_definition_list(
                    definition_type)
                if definition_list:
    ...
    def get_policy_definition_dict(self,
        ...
        return self.list_to_dict(policy_definition_list,
                                 key_name,
                ...
                for entry in sequence['match']['entries']:
                    if 'listName' in entry:
                        ...
                        if entry['listName'] in policy_list_dict:
                            entry['ref'] = policy_list_dict[
                                entry['listName']]['listId']
                            entry.pop('listName')
                            entry.pop('listType')
                        else:
                            raise Exception(
                                "Could not find list {0} of type {1}".format(
    ...
    def import_policy_definition_list(self,
        ...
        for definition in policy_definition_list:
            ...
            if definition['name'] in policy_definition_dict:
                existing_definition = policy_definition_dict[
                    definition['name']]
                if 'defaultAction' in payload:
                    diff.extend(
                        list(
                            dictdiffer.diff(
                                existing_definition['defaultAction'],
                                payload['defaultAction'])))
                if 'sequences' in payload:
                    diff.extend(
                        list(
                            dictdiffer.diff(existing_definition['sequences'],
                                            payload['sequences'])))
                if 'definition' in payload:
                    diff.extend(
                        list(
                            dictdiffer.diff(existing_definition['definition'],
                                            payload['definition'])))
                if len(diff):
                    if 'definition' in definition:
                    ...
                    if not check_mode and update:
                        response = self.update_policy_definition(
                            definition, policy_definition_dict[
