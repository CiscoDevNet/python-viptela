...
def run_module():
    ...
    if vmanage.params['state'] == 'present':
        ...
        security_policy_dict = vmanage_security_policy.get_security_policy_dict(remove_key=False)
        for policy in policy_list:
            if policy in security_policy_dict:
                if not module.check_mode:
                    vmanage_security_policy.delete_security_policy(security_policy_dict[policy['policyName']]['policyId'])
                vmanage.result['changed'] = True
